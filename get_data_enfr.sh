...
for FILENAME in news*gz; do
  ...
  if [ ! -f "$OUTPUT" ]; then
    ...
    # gunzip -c $FILENAME > $OUTPUT  # for gunzip version < 1.6
    gunzip -k $FILENAME
  else
...
if ! [[ -f "$SRC_VOCAB.tf" && -f "$TGT_VOCAB.tf" && -f "$FULL_VOCAB.tf" ]]; then
  ...
  echo -e "<s> 0\n</s> 1\n<unk> 2\n<mask> 3" | cat - $FULL_VOCAB | cut -d' ' -f1 > $FULL_VOCAB.tf 
fi
echo "EN TF vocab in: $SRC_VOCAB.tf"
echo "FR TF vocab in: $TGT_VOCAB.tf"
echo "Full TF vocab in: $FULL_VOCAB.tf"
